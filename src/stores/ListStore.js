...
export default class ListStore {
   ...
   */
  constructor(mode) {
    if (!_includes(ListStore.modes, mode))
      this.triggerError('Selected Mode does not exist');
  ...
  /**
   * Editable observable function provided, for the filters to be modifiable
   * however it is desired
   */
  @observable itemFilterCallback = () => true;

  /**
   * Provides an enumerable from 'modes' freezed variable, which describes what
   * type of items the ListStore provides
   */
  @observable mode;

  /** Error Catch for ErrorBoundary parent component */
  @observable errorBoundary = {hasError : false, error: null};
  errorReaction = reaction(() => this.errorBoundary, (hasError, error) => {
    if (hasError)
      throw new Error(error);
  });

  /**
   * Enumerable for 'mode' variable, describes the type of 'items' provided by
   * the database
   */
  static modes = Object.freeze(
      {PRODUCTS : 'products', COURIERS: 'couriers', CUSTOMERS: 'customers'});

  /**
   * Provides all the necessary constant information about all types of items
   * that can be provided
   */
  @observable
      .ref _modeFields = Object.freeze({
    products : {
      headers : {
        code1 : 'Code1',
        code2: 'Code2',
      ...
      functions: {
        total: (item) =>
            this.sum([ item.gs, item.corfu, item.cclit, item.gp_k ])
      },
      totalColumns: 10,
      columnsWidth: [ 3, 3, 4, 7, 3, 3, 3, 3, 3, 3 ],
      dataTypes: {
        code1: 'string',
        code2: 'string',
        barcode: 'string',
    ...
    couriers: {
      headers: {name: 'Couriers', email: 'Email', phone: 'Phone'},
      api: api.v1.http.couriers
    }
  });

  /**
   * An object describing what the header for each field should be with the keys
   * provided respectively for each field
  ...
  get headers() {
    if (!this.mode)
      this.triggerError('List mode was not defined during item fetching');

    return this._modeFields[this.mode].headers;
  }

  /**
   * A Table shall have a total amount of columns, usually depending on the
   * amount of headers
  ...
  /**
   * The width of each column, so each column takes as much screen space as we'd
   * like
  ...
  /**
   * The type of each field provided for operations like type checking or even
   * frontend decoration for UX
  ...
  /**
   * Provides the API url depending on the current 'mode' - See: 'modes'
   * enumerable
  ...
  get api() {
    if (!this.mode)
      console.log('List mode was not defined during item fetching');
  ...
  @action
      .bound refreshItems() {
    setTimeout(() => this.fetchItems().then(
                   fetchedItems => { this.items = fetchedItems; }),
               10)
  }
  ...
  fetchItems() {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios.get(url, {withCredentials : true})
          .then(action("fetchItems.success", (res) => { resolve(res.data); }),
   ...
   */
  @action
      .bound addItemDb(item) {

    return new Promise((resolve, reject) => {
      if (!item)
        reject('Item parameter was undefined');

      const url = this.api;

      axios.post(url, url, url, item, {withCredentials : true})
          .then(action('addItemDb.success',
                       (res) => {
                         this.fetchItems()
                             .then(action('addItemDb.fetchItems.success',
                                          (fetchedItems) => {
                                            this.items = fetchedItems;
                                            resolve(fetchedItems);
                                          }))
                             .catch(action('addItemDb.fetchItems.success',
                                           (err) => reject(err)));
                       }))
   ...
   */
  @action
      .bound removeItemDb(itemId) {

    return new Promise((resolve, reject) => {
      if (!itemId)
        reject(`Object's ID wasn't given, unable to remove item`);
      ...
      axios.delete(url, {withCredentials : true})
          .then(value => {
            this.fetchItems()
                .then(action('removeItemDb.fetchItems.success',
                             (fetchedItems) => {
                               this.items = fetchedItems;
                               resolve(fetchedItems);
                             }))
                .catch(action('removeItemDb.fetchItems.failure',
                              err => reject(err)));
          })
          .catch(action('removeItemDb.failure', err => reject(err)));
    });
  }
   ...
   * @param {string|number} newValue The new value to be stored to the item's
   * choosen field
   * @returns {Promise<[{fieldName: any}]} Fetched items
   * @memberof ListStore
   */
  @action
      .bound updateItemDb(itemId, fieldName, newValue) {

    return new Promise((resolve, reject) => {
      if (!itemId)
        reject('Error Updating Item: itemId was undefined or null');
      if (!fieldName)
        reject('Error Updating Item: fieldName was undefined or null');
      if (!newValue)
        resolve(
            'Acceptable but item not updated - New Value is null or undefined');

      const updatedItemIndex = _findIndex(this.items, {_id : itemId});
      this.items[updatedItemIndex][fieldName] = newValue;

      const url =
          `${this.api}/${itemId}?${fieldName.toLowerCase()}=${newValue}`;

      setTimeout(() => {
        axios({method : 'put', url, withCredentials : true})
            .then(action('updateItemDb.success'), (value) => resolve(value))
            .catch(action('updateItemDb.failure'), err => reject(err));
      }, 100);
    });
  }

  /**
   * Filters the `items` object with the `itemFilterCallback` observable
   * function
  ...
  /**
   * Filters the `items` object with the `itemFilterCallback` observable
   * function and then places them in a page accordinge to the page rules
   * @returns {[{fieldName: (string|number)}]} Filtered Items
   * @memberof ListStore
   */
  @computed
  get filteredPagedItems() {
  ...
  get totalPages() {
    const totalPages =
        Math.floor(this.filteredItems.length / this.itemsPerPage) + 1;
  ...
  get objectKeys() {
    ...
    return _filter(headerKeys,
                   (header) => { return !this.isFieldFunction(header); });
  }
